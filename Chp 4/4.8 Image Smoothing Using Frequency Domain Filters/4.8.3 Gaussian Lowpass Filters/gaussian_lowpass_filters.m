...
% Applying filters
F1_filtered = F .* get_gaussian_lowpass_filter(size(F), 10);
F2_filtered = F .* get_gaussian_lowpass_filter(size(F), 30);
F3_filtered = F .* get_gaussian_lowpass_filter(size(F), 60);
F4_filtered = F .* get_gaussian_lowpass_filter(size(F), 160);
F5_filtered = F .* get_gaussian_lowpass_filter(size(F), 460);
...
%% Get gaussian lowpass filter
function filter = get_gaussian_lowpass_filter(filter_size, radius)
    p = filter_size(1);
    q = filter_size(2);
    ...
    for u=1:filter_size(1)
        for v=1:filter_size(2)
            filter(u, v) = 1/exp((((u-p/2)^2 + (v-q/2)^2)^2)/(2*radius^2));
        end
    end
end
