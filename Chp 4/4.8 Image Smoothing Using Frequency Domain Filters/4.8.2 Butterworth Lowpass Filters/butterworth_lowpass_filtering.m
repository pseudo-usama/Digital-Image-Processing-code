...
% Applying filters
F1_filtered = F .* get_butterworth_lowpass_filter(size(F), 10, 2);
F2_filtered = F .* get_butterworth_lowpass_filter(size(F), 30, 2);
F3_filtered = F .* get_butterworth_lowpass_filter(size(F), 60, 2);
F4_filtered = F .* get_butterworth_lowpass_filter(size(F), 160, 2);
F5_filtered = F .* get_butterworth_lowpass_filter(size(F), 460, 2);
