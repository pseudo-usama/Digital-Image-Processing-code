...
% Applying filters
F1_filtered = F .* get_ideal_highpass_filter(size(F), 10);
F2_filtered = F .* get_ideal_highpass_filter(size(F), 30);
F3_filtered = F .* get_ideal_highpass_filter(size(F), 60);
F4_filtered = F .* get_ideal_highpass_filter(size(F), 160);
F5_filtered = F .* get_ideal_highpass_filter(size(F), 460);
...
%% Get Ideal highpass filter
function filter = get_ideal_highpass_filter(filter_size, radius)
    p = filter_size(1);
    q = filter_size(2);
