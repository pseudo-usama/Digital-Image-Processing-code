...
% Applying filters
F1_filtered = F .* get_gaussian_highpass_filter(size(F), 10);
F2_filtered = F .* get_gaussian_highpass_filter(size(F), 30);
F3_filtered = F .* get_gaussian_highpass_filter(size(F), 60);
F4_filtered = F .* get_gaussian_highpass_filter(size(F), 160);
F5_filtered = F .* get_gaussian_highpass_filter(size(F), 460);

% Converting back to spatial domain
f1_filtered = uint8(real(ifft2(fftshift(F1_filtered))));
f2_filtered = uint8(real(ifft2(fftshift(F2_filtered))));
f3_filtered = uint8(real(ifft2(fftshift(F3_filtered))));
f4_filtered = uint8(real(ifft2(fftshift(F4_filtered))));
f5_filtered = uint8(real(ifft2(fftshift(F5_filtered))));
...
%% Get gaussian highpass filter
function filter = get_gaussian_highpass_filter(filter_size, radius)
    p = filter_size(1);
    q = filter_size(2);
    ...
    for u=1:filter_size(1)
        for v=1:filter_size(2)
            filter(u, v) = 1 - 1/exp((((u-p/2)^2 + (v-q/2)^2)^2)/(2*radius^2));
        end
    end
end
